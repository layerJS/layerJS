...
var ScriptView = GroupView.extend({
  ...
  /**
   * Will render the scriptView. When WL.executeScriptCode == false
  ...
  render: function(options) {
    ...
    if ('htmlAttributes' in diff) {
      for (var htmlAttribute in diff.htmlAttributes) {
        if ((WL.executeScriptCode || 'src' !== htmlAttribute) && diff.htmlAttributes.hasOwnProperty(htmlAttribute)) {
   ...
   * children array. When WL.executeScriptCode == false, nothing is done.
   *
