...
// TODO finished, just need to delete all the comments that are not relevant + beautify and save to remote repository
...
describe('attribute mutations:', function() {
  ...
  xdescribe('data-lj-id and lj-id and lj-name changes according to id,', function() {
    //failed data-lj-id
    it('change data-lj/lj-id frame1 to frame10 and check that lj-name gets the new lj-id as a name', function() {
      browser.get('attributemutations/attribute_mutations_id.html').then(function() {
        ...
        // when not setting a name the name is the id == failed / passed
        // and if data-lj/lj-id is set then name should be that.
        ...
        protractor.promise.all([
          ...
          var stateExportStrct = data[0];
          expect(stateExportStrct).toEqual(['stage', 'stage.layer', 'stage.layer.tmpId']); //passed 26062017 1221

          // preperation for testing changing data-lj-id/ lj-id
          var prefix = ['data-lj-', 'lj-'];
          for (var i = 0; i < prefix.length; i++) {
            var frdataLjAtt = prefix[i] + 'id'; //once data-lj-, once lj-    //'data-lj-id'
            var frNewLjId = {};
            frNewLjId[frdataLjAtt] = 'frame1';

            // setting lj-id to frame1
            utilities.setAttributeByClassName('fr', frdataLjAtt, frNewLjId[frdataLjAtt]).then(function() {
            //option 2 for same action
            //  utilities.setAttribute('tmpId', frdataLjAtt, frNewLjId[frdataLjAtt]).then(function() {
              protractor.promise.all([
                utilities.getCurrentExportStructure('layer')
              ]).then(function(data) {
                var stateExportStrct = data[0];
                expect(stateExportStrct).toEqual(['stage', 'stage.layer', 'stage.layer.frame1']); //failed 26-06-2017

                // test after changing data-lj/lj-id if data-lj-name changes to new lj-id value.
                frNewLjId[frdataLjAtt] = 'frame10';
                utilities.setAttributeByClassName('fr', frdataLjAtt, frNewLjId).then(function() {
                  protractor.promise.all([
                    utilities.getCurrentExportStructure('layer')
                  ]).then(function(data) {
                    var stateExportStrct = data[0];
                    expect(stateExportStrct).toEqual(['stage', 'stage.layer', 'stage.layer.frame10']); //failed 26-06-2017

                    // remove the attributes to allow a reset for next iteration in the for loop
                    utilities.removeAttribute('frame10', 'data-lj-id');
                    utilities.removeAttribute('frame10', 'lj-id');
                  });
                });
              });
            });
    ...
    // passed. 09-05-2017
    xit('change id frame1 to frame10 and check that lj-name gets the new id as a name', function() {
      browser.get('attributemutations/attribute_mutations_id.html').then(function() {
  ...
  // layout-type -
  // switch to canvas:
    // show inactive frames at right pos
    // no change visible in active frame
  // switch to slide:
    // hide inactive frames
    // no change visible in active frame
  // *defined in the layer
  // **Determines in which layout the frames
  // are shown. Possible values: slide, canvas.

  // data-lj-/lj-layout-type
  describe('data-lj-layout-type and lj-layout-type,', function() { //TODO u r here
    ...
    it('change undefined layout-type to canvas', function() {
      browser.get('attributemutations/attribute_mutations_layout_type.html').then(function() {
        ...
        for (var i = 0; i < prefix.length; i++) {
          ...
          // test that layout-type has changed (if for data-lj- or lj-) //passed 09-05-2017
          if (i == 0) {
            protractor.promise.all([
    ...
    it('change layout-type canvas to slide', function() {
      browser.get('attributemutations/attribute_mutations_layout_type.html').then(function() {
        ...
        var f2 = element(by.id('frame2'));
        var prefix = ['data-lj-', 'lj-'];
        var layoutType = {}; // TODO check if makes a change cuz shouldnt - moved it from inside the loop
        ...
        for (var i = 0; i < prefix.length; i++) {
          ...
          if (i == 0) {
            ...
            protractor.promise.all([
              utilities.getAttribute('layer', 'lj-layout-type'),
              utilities.getStyle("frame2", "display"), //getting css disply f2
               utilities.getBoundingClientRect("frame2")
            ]).then(function(data) {
              var att = data[0];
              var fr2Display = data[1];
               var frame2_dimensions = data[2];

              expect(att).toEqual('canvas'); //    passed 09-05-2017
              expect(fr2Display).toBe('block'); // fails and is a bug/issue(checked w/ Thomas) - happens because transitioned from defined slide layout and not default slide layout.  03-07-2017

              // console.log(frame2_dimensions.display);
              expect(frame2_dimensions.display).toBe('block'); // FIXME Failed:  Expected undefined to be 'block'. 04-07-2017
  ...
  xdescribe('data-lj-native-scroll and lj-native-scroll,', function() {
    ...
    it('change data-lj/lj-native-scroll', function() {
      browser.get('attributemutations/attribute_mutations_native_scroll.html').then(function() {
        ...
        // function to test that 'layer' has 'lj-helper' child element
        //  function childHasAttribute(elementId, attribute) {
        //    var el = element(by.id(elementId));
        //    //document.getElementById(elementId);
        //        return el.children[0].hasAttribute(attribute);
        //    element(by.id('layer')).children[0].hasAttribute('lj-helper')
        //        return el
        //    }
        ...
        for (var i = 0; i < prefix.length; i++) {
          ...
          //  var hasHelper = childHasAttribute('layer', 'lj-helper');//FIXME --== failed 19062017

          // test that 'layer' has 'lj-helper' child element
          protractor.promise.all([
            utilities.childHasAttribute('layer', 'lj-helper') //FIXME --== failed 19062017 when function not in utilities file
          ]).then(function(data) {
            var hasHelper = data[0];
            console.log(i, "after promise");

            expect(hasHelper).toBe(true); // passed as function in utilities. 10-05-2017 - but as default, when no native-scroll is the defined, it is defined as native-scroll='true' and so will have the lj-helper even when native-scroll is not defined manually.
            console.log(i); //, " ************************   iterated over true passed ************************"
            // var tChildAtNull = element(by.id('layer')).children[0];
            // var tElement = element(by.id('layer'));
            // console.log("tElement : ", tElement);
            // console.log("tChildAtNull : " + tChildAtNull);
            // expect(element(by.id('layer')).children[0].hasAttribute('lj-helper')).toBe(true); //FIXME

            nativeScroll[frdataLjAtt] = false;

            utilities.setAttributes('layer', nativeScroll).then(function() {
              utilities.wait(2000);
              // tChildAtNull = element(by.id('layer')).children[0];
              // tElement = element(by.id('layer'));
              // console.log("tElement : "+ tElement);
              // console.log("tChildAtNull : " + tChildAtNull);
              // console.log("json : ");
              // console.log(JSON.stringify(tElement));
              //expect(element(by.id('layer')).children[0].hasAttribute('lj-helper')).toBe(false); //FIXME
              ...
              protractor.promise.all([
                ...
                expect(hasHelper).toBe(false); //  failed 10-05-2017 as function in utilities - result: Expected true to be false.
                console.log(i, " ************************  iterated over false ************************");
...
failed Tests(and reasons if known):
- id change lj-id data-lj-id - in state.js function: _attributesChangedEvent(error:Failed: javascript error: Cannot read property 'path' of undefined) + in base.js changed with Thomas function id:
-
