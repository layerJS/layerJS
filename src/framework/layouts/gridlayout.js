...
var GridLayout = LayerLayout.extend({
  ...
  _calculateTransform2: function(frame, transformData, top, left) {
    ...
    var transform = "translate3d(" + (parseInt(transformData.shiftX, 10) + left).toString() + "px," + parseInt(transformData.shiftY, 10) + top + "px,0px) scale(" + (transformData.scale || 1) / (frame.scaleX() || 1) + "," + (transformData.scale || 1) / (frame.scaleY() || 1) + ") rotate(" + (rotation || 0) + "deg)";
    return transform;
  },
  /**
   * calculate the transform that transforms a frame into the stage
